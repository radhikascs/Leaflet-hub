...
L.ChartMarker = L.FeatureGroup.extend({
    ...
    _bindMouseEvents: function(chartElement) {
        ...
        chartElement.on("mouseover", function(e) {
			...
			displayText = dureUtil.numberWithCommas(displayText);
			console.log('Hardcoded for hover on chart overlay');
            legendOptions[key] = {
...
//				console.log('Inside COUNTRY');
        var code = this.options.codeField ? L.Util.getFieldValue(record, this.options.codeField) : index;
       // console.log("this is code in dvf file");
       // console.log(code);
        var geoJSON;
        var centroid;
	...
	DISTRICT: function(record, index) {
		console.log("inside district");
		//debugger;
        var code = this.options.codeField ? L.Util.getFieldValue(record, this.options.codeField) : index;
        var geoJSON;
        var centroid;
        var district = dureOverlays.formatDistrictGeoJson(subprovince.geoJson) || {};
        //var stateCentroids = L.stateCentroids || {};
        var lDistrictCentroids = L.GeometryUtils.loadCentroids(district);
		var districtCentroids = lDistrictCentroids || {};
		
        var originalCode = code.toUpperCase();
        code = originalCode;
        geoJSON = district[code];
		//console.log(geoJSON);
        centroid = districtCentroids[code];
        var geoJSONLayer = new L.GeoJSON(geoJSON);
        return {
            location: geoJSONLayer,
            text: L.GeometryUtils.getName(geoJSON) || code,
    ...
    _getDynamicOptions: function(record) {
        ...
        if (displayOptions) {
            for (var property in displayOptions) {
				...
				displayText = dureUtil.numberWithCommas(displayText);
				//console.log('Hardcoded for hover on bubble overlay');
