...
iHealthMap.loadMapTiles = function(){
	...
	var osmUrl='https://cartocdn_{s}.global.ssl.fastly.net/base-eco/{z}/{x}/{y}.png';
	var osm = new L.TileLayer(osmUrl, {minZoom:iHealthMap._defaultZoom , maxZoom: 12, attribution: "Dure Technologies@2014"});		
...
iHealthMap.rangeCompare = function(rangeData,valueTocompare, returnStyleObj){
    ...
    for(var i = 0;i<rangeData.lowScale.length;i++){
   	...
   	if (valueTocompare == -1 && dureUtil.indicatorId != 165) {                   // temp fix change it later
...
iHealthMap.renderFilter = function () {
    ...
    /* empty the container for new fileter data */
    $('#accordion').empty();
    ...
    for (var i = 0; i < filterGroupsData.length; i++) {
        ...
        for (var j = 0; j < filterGroupsData[i].filterSubGroups.length; j++) {
            ...
            '</div>'; /*  Internal main panel ends here */
        }
            ...
            $.each(checkedFilters, function (key, val) {
                ...
                if ($(val).hasClass('baseFiltercls')) {
                    ...
                    if (dureUtil.respJsonData.indicatorMetaInfo[0].indicatorInfo.indicatorId == 1152) {
                        ...
                        if (val.getAttribute("dataheadervalue") == 'Introduced') { /* create Introduced legend */
                        ...
                        } else if (val.getAttribute("dataheadervalue") == 'Planning') { /* create Planning legend */
                        ...
                        } else if (val.getAttribute("dataheadervalue") == 'Not Introduced') { /* create Not introduced legend */
                        ...
                        if (val.getAttribute("dataheadervalue") == 'N/A') { /* create Introduced legend */
                    ...
                    if(iHealthMap.stylingdata[iHealthMap.getCurrentyear()]){
						...
						tempSingles.sort(function(a, b){return a-b});
						var objectSingles = [];
...
tempval = (jQuery.inArray($(arr[i][0][l]).val(), v[0]));
        ...
        if (tempval < 0) {
            ...
            filter.truth.push(false);filter.truth
        } else {
...
iHealthMap.reloadBaseLayer = function(){
	...
	setTimeout(function(){ iHealthMap.map.invalidateSize()}, 400);
};
