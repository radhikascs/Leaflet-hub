...
// To fetch impact study data
// @ Author- Radhika 
dureUtil.getImpactStudiesData = function (indicatorValue) {
	console.log(indicatorValue);
	if (indicatorValue = 182) {
		impactStudies.queryString = 'appid=' + dureUtil.appId + '&langid=' + dureUtil.langId + '&typeid=1';
	}
	else if (indicatorValue = 183) {
		impactStudies.queryString = 'appid=' + dureUtil.appId + '&langid=' + dureUtil.langId + '&typeid=2';
	}
	else{
		impactStudies.queryString = "";
	}
	impactStudies.url = dureUtil.AppBaseURLContext + 'dataapi/impactstudydata?' + impactStudies.queryString;
	console.log(impactStudies.url);
	var serviceObj = {
		type: 'GET',
		url: impactStudies.url,
		dataType: 'jsonp',
		contentType: 'application/json',
		crossDomain: true,
		success: function (resp) {
			dureUtil.impactStudiesValues = resp;
		}
	}
	$.ajax(serviceObj);
...
// Function response for list of impact Studies
/*dureUtil.callImpactStudiesService = function(){

	if(dureUtil.checkIfKeyExsist('ImpactStudies')) {

		resp = dureUtil.retrieveFromLocal('ImpactStudies');
		callback_GetRegionList(resp, 'local')
	} else {
	impactStudies.queryString = 'appid=' + dureUtil.appId + '&langid=' + dureUtil.langId + '&typeid=1';
	impactStudies.url = dureUtil.AppBaseURLContext + 'dataapi/impactstudydata?' + impactStudies.queryString;
		...
		if(resp.indicatorMetaInfo != undefined) {
			...
			if(resp.indicators.length != 0) {
				...
				worldInd.extractedObjects = dureUtil.extractDataObjects(resp.indicators);
                                worldInd.indicatorMetaInfo=dureUtil.indicatorMetaInfo;
